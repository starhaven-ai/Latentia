  ...
  return prisma.$transaction(async (tx) => {
    const jobs = await tx.generationJob.findMany({
      where: {
        ...
        AND: [{ OR: [{ runAfter: null }, { runAfter: { lte: now } }] }],
      },
...
async function processGenerationById(generationId: string): Promise<GenerationProcessResult> {
  ...
  try {
    ...
    if (result.status === 'completed' && result.outputs) {
      ...
      await prisma.output.createMany({ data: outputRecords })
